...
| `chain_format` | Signing format number (currently `5`; absent on pre-format-2 events) |
| `prev_sig` | Signature of the previous event (chain link) |
| `sdk_sig` | HMAC-SHA256 over `format|session|seq|timestamp|content_hash|decision_hash|prev_sig` (`decision_hash` is present in formats 3–5; formats 1–2 omit it) |
...
- **Client signing alone is not non-repudiation against a key-holder.** The SDK signing key is derived from the API key, so a party holding the API key could construct validly-signed *client* chains. This is why ingest adds a server countersignature with a key that never leaves the server: a key-holder cannot forge it, so they cannot fabricate an event that appears to have been accepted. What the client chain still cannot prove is (a) what happened *inside your process before emission*, or (b) the integrity of fields left outside its preimage. In **chain format 5**, the decision digest covers the format-3 decision fields, format-4 classification fields, and `source_lineage_hash`. **The lineage envelope remains in metadata and is validated against that signed hash; `tenant_id`, token counts, cost, and unrelated metadata remain outside the client preimage.** The server countersignature over the full canonical event seals those fields once accepted.
...
### Source-lineage security boundary

Source lineage is a signed application assertion, not a heap scanner or a
content-origin detector. Both SDKs validate and canonicalize the same bounded
`obsvr-source-lineage/1` envelope, then chain format 5 seals its hash.

| Claim | What supports it | What it does not prove |
| --- | --- | --- |
| This event carried source revision X | exact `source_hash` or immutable `source_version` inside the envelope whose hash is signed | that the application supplied every source it used |
| This lineage derived from Y | direct `parent_lineage_ids`, propagated by the caller across each derivation | semantic causation or influence on the output |
| A detector marked this source risky | bounded taint with kind, reason, time, and optional detector/trigger event | that every use was unsafe or that detection happened on first use |
| These earlier events are in the retrospective blast radius | their signed envelopes reference the same exact source revision or a recorded descendant | that the risk was known when those events originally ran |

Ambient propagation covers only the current `AsyncLocalStorage` /
`contextvars` execution. Queue, process, and separately scheduled worker
boundaries require explicit envelope export and validation on re-entry. A bare
mutable source ID is weaker than a content hash: if revisions reuse the ID, a
retrospective query cannot distinguish them without `source_hash` or an
immutable version. Arbitrary metadata is not signed field-by-field; verification
recomputes the canonical envelope and requires its hash to equal the signed
`source_lineage_hash`.
...
LangChain still requires its callback handler. TypeScript LlamaIndex model calls
are automatic only when the app assigns a compatible model through the root
`Settings.llm` object obtained after preload; tracing remains observe-only and
agent tools remain explicit. Hosted provider-side tools expose no local
callback to attach, and OpenAI Agents MCP tools converted per turn must be
governed at the MCP boundary.

Therefore an instance, constructor, `Settings` object, tool, or callable captured before startup
...
After application factories finish, exact coverage requirements can reject a
missing symbol or insufficient enforcement depth. A separate deny smoke helper
requires the caller's selected factory path to reject while its downstream
transport counter stays unchanged. Together these are deployment checks for a
named path, not discovery of every client in the process.

### Durable audit outbox

Durable delivery is opt-in. The outbox writes one signed event per file using an
atomic rename before enqueue returns, replays pending records on restart, removes
them only after accepted delivery, and moves permanent failures to `dead/`.

| Property | Boundary |
| --- | --- |
| Default | disabled; the existing bounded in-memory queue remains the default |
| Persistence failure | throws in `error` mode; `warn` mode explicitly falls back to memory-only delivery |
| Storage | must be an absolute, non-symlink, private directory; use one directory per process |
| Confidentiality | outbox files can contain governed prompt, response and decision data; filesystem encryption, retention and deletion are operator responsibilities |
| Completeness | recovers events already emitted and persisted; cannot discover bypassed calls or survive loss of the outbox volume |
| Capacity | `maxBytes` / `max_bytes` is a per-process guard, not a distributed quota |
...
Terminal-outcome upload is separate from execution and recovery, and it is never automatic. The application must explicitly submit the signed outcome or terminal journal; a saved terminal journal can be submitted after restart. Before sending, the SDK verifies the outcome signature, execution-authorizing receipt signature, signer identity, and all bindings. The transport uses bounded DNS resolution, a pinned address snapshot, no redirects, HTTPS except loopback, bounded request and response sizes, a retry deadline, and the outcome hash as the idempotency key. Only an exact `400`, `401`, `403`, or `413` rejection with the matching outcome hash and `stored: false` is classified as `definitive_no_store`. A `409`, malformed response, redirect, timeout, retry exhaustion, or other ambiguous result remains `uncertain`. No delivery result relabels an executed, failed, or uncertain local outcome.

Hosted strict ingest can add a server acceptance attestation to each accepted decision, resolution, or terminal outcome. This is a distinct Ed25519 signature over the record type, tenant, evidence hash, acceptance time, and SDK signer identity. It does not claim that the server signed every field of the strict envelope; the canonical accepted envelope is preserved separately. Active and retired server keys may be pinned for verification.

Hosted strict admission also projects applicable incidents atomically with accepted evidence. A `DENY` decision projects a high-severity policy violation; a terminal `failed` outcome projects a medium-severity system error; a terminal `uncertain` outcome projects a high-severity system error. Successful outcomes and approval-denied resolutions do not create incidents. These projections are hosted-service behavior, not part of the public SDK implementation.

A strict evidence bundle is a separate portable envelope containing a contiguous trusted receipt chain, zero or one trusted terminal outcome per authorized decision, coverage status, and reconstructed policy continuity. It rejects partial chains, foreign or duplicate outcomes, unknown keys, unsupported fields, invalid signatures, and a bundle signer that does not match the head receipt signer. `complete` means every authorized decision in that supplied chain has a terminal outcome; it does not prove the supplied chain includes every action that ever occurred outside it. The SDK bundle does not currently include the hosted server acceptance attestation, a daily Merkle inclusion proof, or an off-host anchor; those are separate hosted evidence layers.
...
- **Pre-call policies are real enforcement on the documented enforcing surfaces.** The direct-client wrappers, provider/infra adapters, MCP boundary, explicit tool governors, LangChain model-start callbacks, Python LlamaIndex model-start callback, TypeScript models assigned through the intercepted LlamaIndex `Settings.llm`, and explicit OpenAI Agents/LlamaIndex model wrappers run their applicable PII, rule, and hook layers before dispatch, so a blocked call never leaves that boundary. TypeScript LlamaIndex tracing and OpenAI Agents tracing remain observe-only and are not included in this guarantee.
...
- **Session taint latch escalates a compromised session's later egress.** With `sessionTaint: { enabled: true }` (both SDKs), once a prompt-injection or a canary leak is detected in a session, that session is marked tainted and its subsequent egress — LLM calls, tool-call arguments, MCP calls — is escalated: `action: "flag"` (the default) annotates the events (`rule_id: sdk:session_tainted`) so the compromise is visible without bricking the session; `action: "block"` refuses the tainted session's egress. This is a session-level LATCH, not data-flow label propagation: it does not tag individual values and follow them, it records that a session id is compromised and remembers it (the per-call scanners can miss a staged exfiltration; the latch does not). **`destructiveTools` is a list of EXACT tool names a tainted session may never invoke, and where the SDK is genuinely on the tool boundary it does exactly what the design intends** — ordinary egress in a tainted session stays merely flagged, while the capabilities that could do damage (`send_money`, `delete_records`, ...) go dark, under `action: "flag"` and not only under `block`. **It is on fewer boundaries than that reads, and the honest statement is per surface rather than per SDK — so it is stated per surface:**
  ...
  - **A tool boundary the SDK was NOT on, and now is:** a provider tool runner (`chat.completions.runTools`, `beta.messages.toolRunner`) invokes its tools itself, so `destructiveTools` did not reach them — measured, a tainted session executed `send_money` there under `flag`. obsvr now gates each local callback before registration and each supported model turn before its provider request. A hosted tool the provider executes on its own infrastructure has no local callback to gate; see "Bypass surface" for that limit.
...
- **Provider tool runners gate every supported local turn and local tool.** TypeScript `chat.completions.runTools` and `beta.messages.toolRunner`, plus Python's supported Anthropic runners, retain governed request state so each later model request is checked before transport. Local callbacks are wrapped before the provider snapshots the tool set, so denied tools, allowlists, and `destructiveTools` prevent the callback body from running. Two limits remain. Hosted or server-side tools expose no local callback to wrap and are named in `tool_gate_ungated_tools`. A block on a later model turn prevents that request but cannot undo a tool side effect that completed on an earlier allowed turn. Refusal shape still follows the provider: Anthropic can convert a local-tool refusal to an error tool result, while OpenAI propagates it and ends the run.
...
- **Observation rails are named rather than grouped with enforcing callbacks.** TypeScript LlamaIndex tracing and OpenAI Agents tracing run the storage-only PII pass on model calls and record `not_evaluated`; they cannot stop provider dispatch. LangChain model-start callbacks enforce in both SDKs, Python's LlamaIndex callback enforces, and explicit LlamaIndex/OpenAI Agents model wrappers own the underlying model call and can rewrite provider-bound content. Use one of those enforcing boundaries when a block or redaction must affect the wire.
- **Named TypeScript compatibility wrappers use the generic governed-path table.** `wrapAzureOpenAI`, `wrapTogether`, and `wrapOpenAICompatible` delegate to `obsvr.wrap()`'s method resolver and enforcement pipeline, adding endpoint attribution rather than a narrower one-method gate. Legacy `completions.create` and Responses compaction are governed; batch and assistants surfaces remain outside the ordinary method table.
