...
| `chain_format` | Signing format number (currently `4`; absent on pre-format-2 events) |
| `prev_sig` | Signature of the previous event (chain link) |
| `sdk_sig` | HMAC-SHA256 over `format|session|seq|timestamp|content_hash|decision_hash|prev_sig` (`decision_hash` is present in formats 3–4; formats 1–2 omit it) |
...
- A third party can verify the chain independently of obsvr using the exported events and the public verification tooling (`obsvr-verify`, shipped by **both** SDKs with identical modes, exit codes, and verdicts — a Python-only shop needs no Node toolchain to check its own evidence, and CI drives both binaries over one export to keep them from drifting). This offline check attests prompt/response **content** integrity, event **order**, and — since chain format 3 — the **decision fields** (`action_taken`, `action_reason`, `reason_code`, `rule_id`, `policy_version`, `model`, `provider`, `user_id`). Format 4 also seals the record-classification fields `operation`, `source`, and `event_type`, preventing an ordinary signed event from being relabeled as `audit.gap`. `tenant_id`, token counts and cost remain outside the client preimage and are covered by the server countersignature. Earlier chains verify at their original strength; the verifier reports which format it checked.
