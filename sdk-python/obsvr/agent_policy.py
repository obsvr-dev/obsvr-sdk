...
def check_steps(count: int, policy: Dict[str, Any]) -> Tuple[str, Optional[str]]:
    ...
    FAILS CLOSED on a disposition it does not recognise. It previously returned
    ``policy["step_limit_action"]`` verbatim, so a value the callers do not
    implement — a typo like ``"warn"``, or a disposition from a newer config
    than the installed SDK — matched neither branch at the call site and the
    limit did nothing at all, with nothing recorded either, so a run that blew
    straight through its budget looked identical to one that stayed inside it.
    Measured live: with ``max_steps: 2`` and an unrecognised action, a chain ran
    five tools.
    ...
    One definition, imported by every integration, replacing four identical
