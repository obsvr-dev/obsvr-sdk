"""Read-only, DNS-pinned reconciliation for a prepared strict v2 receipt."""
...
STRICT_RECONCILIATION_V2_SCHEMA = "obsvr-strict-receipt-reconciliation-v2"
STRICT_RECONCILIATION_V2_ENDPOINT = "/ingest/strict-receipts/v2/reconcile"
...
class StrictReconciliationV2Result:
...
def _base(receipt, attempts, status, reason=None):
    ...
    if reason is not None:
        value["reason"] = reason
    return StrictReconciliationV2Result(value)
...
def _validate_receipt(receipt):
    ...
    if (
        ...
        raise ValueError("receipt must be a valid strict v2 envelope")


def reconcile_strict_receipt_v2(
    ...
    for attempts in range(1, max_attempts + 1):
        try:
            ...
            if (
                ...
                and value.get("schema") == STRICT_RECONCILIATION_V2_SCHEMA
...
def assert_accepted_strict_reconciliation_v2(result, receipt):
    value = result.value if isinstance(result, StrictReconciliationV2Result) else {}
    if (
        ...
        raise ValueError("trusted accepted reconciliation is required")
