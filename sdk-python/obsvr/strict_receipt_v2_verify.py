"""Offline verification for versioned strict v2 receipts and chains."""
...
from .strict_receipt_v2 import (
    ...
    strict_receipt_v2_signature_preimage,
)
from .tool_pinning import _canonical_json_for_hash

_HEX = frozenset("0123456789abcdef")
StrictV2KeyTrust = Literal[
...
def _decode_public_key(value: Any) -> Optional[bytes]:
    ...
    try:
        ...
        return (
            raw if len(raw) == 32 and base64.b64encode(raw).decode() == value else None
        )
    except (binascii.Error, ValueError):
        return None


def verify_strict_receipt_v2(
    ...
    schema_valid = bool(
        ...
        and envelope.get("schema") == STRICT_RECEIPT_V2_ENVELOPE_SCHEMA
    ...
    if body is not None:
        try:
            normalized = build_strict_receipt_v2_body(body)
            semantic_valid = _canonical_json_for_hash(
                normalized
            ) == _canonical_json_for_hash(body)
        except StrictReceiptV2ValidationError:
            pass
    hash_valid = False
    if (
        body is not None
    ...
    key_trust: StrictV2KeyTrust = (
        ("pinned" if pinned is not None else "unknown")
        if pin_supplied
        else ("self_asserted" if embedded else "unknown")
    )
    ...
    fingerprint = strict_receipt_v2_key_id(raw_key) if raw_key is not None else None
    identity_valid = bool(
...
def _same_action(left: Dict[str, Any], right: Dict[str, Any]) -> bool:
    return all(
        ...
        for field in ("action_id", "kind", "name", "arguments_hash", "target_hash")
    )


def _compatible_resolution(suspension: Dict[str, Any], method: str) -> bool:
    if suspension.get("type") == "approval":
        return method in ("approval_granted", "approval_denied", "expired", "cancelled")
    return method in ("context_supplied", "expired", "cancelled")


def _compatible_resolution_outcome(method: str, outcome: str) -> bool:
    if method in ("approval_denied", "expired", "cancelled"):
        return outcome == "DENY"
    if method == "approval_granted":
        return outcome in ("ALLOW", "MODIFY")
    return outcome in ("ALLOW", "DENY", "MODIFY")


def verify_strict_receipt_v2_chain(
    ...
    for index, candidate in enumerate(envelopes):
        ...
        receipt_id = (
            body.get("receipt_id", f"index-{index}") if body else f"index-{index}"
        )
        ...
        if (
            ...
            continue
        body = build_strict_receipt_v2_body(body)
        if tenant is None:
            tenant = body["tenant_id"]
        if session is None:
            session = body["session_id"]
        if agent_id is None:
            agent_id = body["initiator"]["agent_id"]
        if key_id is None:
            key_id = body["initiator"]["key_id"]
        if current["receipt_hash"] in seen_receipts:
            errors.append(f"duplicate_receipt:{receipt_id}")
        else:
            seen_receipts.add(current["receipt_hash"])
        ...
        if body["record_type"] == "resolution" and resolution:
            ...
            if prior is None:
                ...
                if prior_body["record_type"] != "decision" or suspension is None:
                    ...
                    if not _compatible_resolution_outcome(
                        resolution["method"], body["evaluation"]["outcome"]
                    ):
                        errors.append(f"resolution_outcome_mismatch:{receipt_id}")
                    if (
                        resolution["resolved_at_ms"] < prior_body["timestamp_ms"]
                        or body["timestamp_ms"] < prior_body["timestamp_ms"]
                    ):
                        errors.append(f"resolution_time_invalid:{receipt_id}")
                    if resolution["method"] in (
                        "approval_granted",
                        "context_supplied",
                    ) and (
                        resolution["resolved_at_ms"] >= suspension["expires_at_ms"]
                        or body["timestamp_ms"] >= suspension["expires_at_ms"]
                    ):
                        errors.append(f"resolution_after_expiry:{receipt_id}")
                    if resolution["method"] == "expired" and (
                        resolution["resolved_at_ms"] < suspension["expires_at_ms"]
                        or body["timestamp_ms"] < suspension["expires_at_ms"]
                    ):
                        errors.append(f"resolution_before_expiry:{receipt_id}")
        ...
        by_hash[current["receipt_hash"]] = {**current, "body": body}
        previous = {**current, "body": body}
