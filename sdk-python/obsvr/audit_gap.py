...
is deliberately NOT in it, so a count carried only in metadata could be edited
from 10,000 to 1 without breaking a single signature - a tamper-evident record
whose one load-bearing number is not tamper-evident. Putting the canonical
statement in the signed content preimage costs no chain-format change and no
verifier special case: the marker verifies exactly like any other event, and
its claim is covered by the same HMAC. The structured copy in
