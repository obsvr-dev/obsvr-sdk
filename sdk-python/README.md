...
4. **Two tool gates refuse nothing, and CrewAI needed its own mechanisms.**
   LlamaIndex has no tool gate of its own — gate the tool itself with
   `obsvr.govern_tool`, or put the capability behind MCP. The OpenAI Agents
   tracing surface cannot refuse structurally: the framework wraps every
   processor callback in its own `try`/`except` and the gate runs after the
   tool has returned. CrewAI's step callback fires only after the step it
   reports, so obsvr refuses AHEAD of it instead: CrewAI's own
   `before_tool_call` hook (1.15.3+, sentinel contract) and the governed tool,
   both driven live to zero side-effect writes on denied tools. Where a
   ...
   `autogen`, `pydantic_ai`, `crewai` (via its hook) and any
   `govern_tool`-wrapped tool are where a tool-policy decision means what it
   says. [Grading](#framework-integrations).
