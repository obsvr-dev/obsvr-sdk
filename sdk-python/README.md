...
4. **One tool gate refuses nothing, and two frameworks needed pre-execution
   mechanisms of their own.** LlamaIndex has no tool gate of its own — gate
   the tool itself with `obsvr.govern_tool`, or put the capability behind
   MCP. CrewAI's step callback and the OpenAI Agents tracing processor both
   fire only after the step they report, so obsvr refuses AHEAD of each:
   on CrewAI, its own `before_tool_call` hook (1.15.3+, sentinel contract)
   and the governed tool; on OpenAI Agents, `attach_tool_gate` — obsvr's
   `ToolInputGuardrail` on each function tool, consulted by the executor
   before invocation, refusing by `reject_content` so the run continues —
   and the governed tool, whose refusal aborts the run as `UserError`. All
   four driven live to zero side-effect writes on denied tools. Where a
   ...
   `autogen`, `pydantic_ai`, `crewai` (via its hook), `openai_agents` (via
   its guardrail) and any `govern_tool`-wrapped tool are where a tool-policy
   decision means what it says. [Grading](#framework-integrations).
