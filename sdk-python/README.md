...
| CrewAI | `integrations.crewai` | `step_callback` / kickoff callbacks | observe (agent run, step, task) + post-run output policy. **Tool policy: not wired** — the gate reads `step.tool`, and the step callback now delivers only a finish record carrying no tool name, by which point the tool has already run. The **step limit is a separate matter and does fire**: it is checked on every step callback rather than inside the tool-name branch, so `max_steps` is reachable on this integration even though the allow-deny list is not. **Measured, and not repaired on purpose:** the installed release does offer a pre-execution, per-tool hook, so the surface exists. Its blocking contract is a returned sentinel, and an exception raised in one of those hooks is swallowed and the tool runs — so wiring this SDK's raise-based idiom to it would fail OPEN, which is worse than the documented silence. A block there is also handed back to the agent as a tool result rather than stopping the run. Adopting that contract is a design decision rather than a port |
...
| LlamaIndex | `integrations.llamaindex` | `BaseCallbackHandler` | observe + stored-copy PII. **No tool gate exists here, and that is settled rather than pending** — this is an observe-only handler, `agent_policy` has no effect on it, and a gate hung off an observability callback is the mistake the rest of this table documents. To refuse a LlamaIndex tool on Python, put the capability behind the MCP integration; the TypeScript SDK can also gate the tool itself with `obsvrGovernTool`, which Python has no equivalent of |
