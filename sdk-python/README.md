...
`messages.create` / `.parse`, their `with_raw_response` variants where the
provider exposes them, `generate_content` / `generate_content_async`, and the `beta.chat.completions.create` / `.parse`,
