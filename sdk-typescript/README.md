...
A successfully finalized invocation records a device-signed terminal outcome bound to the admitted decision, operation fingerprint, attempt, start time, and a result digest or bounded failure code. If terminal signing or persistence fails after execution starts, the durable journal remains unresolved as `invocation_uncertain`; no terminal outcome is claimed. `reconcileStrictRuntimeExecutionV21()` verifies a saved journal and an optional terminal outcome without ever declaring the action retry-safe. `createStrictEvidenceBundleV21()` and `verifyStrictEvidenceBundleV21()` create and verify a portable signed chain with outcome coverage and reconstructed policy continuity.
